...
func LoadRaftConfigFile(filename string) (ipList []string) {
	...
	for index := 0; ; index++ {
		...
		if e != nil && e != io.EOF {
			log.Fatal("SURF_CLIENT_ERROR : Error During Reading Config", e)
		}
...
func ServeRaftServer(server *RaftSurfstore) error {
	...
	fmt.Println("Server with IP : ", server.ip, " is UP")
	go server.commitWorker()
	return s.Serve(l)

	// for _, addr := range server.ipList {
	// 	conn, err := grpc.Dial(addr, grpc.WithInsecure())
	// 	if err != nil {
	// 		log.Fatal("Error connecting to clients ", err)
	// 	}
	// 	client := surfstore.NewRaftSurfstoreClient(conn)
	// 	server.rpcClients = append(server.rpcClients, client)
	// }
